...
MQ 的应用场景主要有三点，分别是：解耦、异步、削峰。

- 解耦：如果两个业务系统之间不需要同步通信的话，可以使用 MQ 进行异步通信，这样即使某个系统挂了，也不会导致整体系统瘫痪，只不过是消息积压在 MQ 中而已。
- 异步：如果某个业务涉及多步操作，而且不需要同步等待时，比如下单后，还要更新库存、更新积分等，可以将一些操作放到 MQ 中，异步去操作，这样可以减少系统的响应时间。
- 削峰：可以将瞬时流量导致的大量消息放到 MQ 中，然后根据我们系统的最大消费能力去消费，这样相当于做了个缓冲，让我们的系统不至于一下瘫痪。

### 2、MQ 的常见问题的解决

MQ 常见的问题有：消息丢失、消息重复消费、消息积压。

- 消息丢失：首先 MQ 是由消息确认机制的，如果消息生产者没有收到 MQ 的确认的话，需要进行消息的重发；然后消息存储阶段，可以通过 MQ 集群和 MQ 持久化来保证消息不会丢失；最后在消息消费阶段，消息消费者需要在接收并处理完消息后，再向 MQ 返回确认，然后 MQ 才能删除消息。
- 消息重复消费：只能业务端自己控制，对于已经消费成功的消息，本地数据库表或者 Redis 缓存一个业务标识，每次处理前先校验，保证幂等。
- 消息积压：先增加消费者数量，把消费能力提升上来；在生产者段进行限流；定位问题，优化消费者逻辑，增加消费速度；处理失败的异常消息加入到死信队列。